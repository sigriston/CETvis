...
dadosFeridas <- aggregate(merged$classificacao, by=list(merged$diasem,merged$dia,merged$escolaridade.y),
                          function(x) sum(x=="F", na.rm=TRUE))

# numero de vitimas mortas por dia, por escolaridade (1 a 8)
dadosMortas <- aggregate(merged$classificacao, by=list(merged$diasem,merged$dia,merged$escolaridade.y),
                         function(x) sum(x=="M", na.rm=TRUE))

# numero de vitimas feridas ou mortas por dia, por escolaridade (1 a 8)
