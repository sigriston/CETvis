    ...
    date.form = "%Y-%m-%d") 
{
    ...
    r2gback <- c("#B5E384", "#FFFFBD","#FFAE63","#D61818" )
    ...
    print(cal.plot <- levelplot(value ~ woty * dotw | yr, data = caldat, 
        ...
        between = list(x = 0, y = c(1, 1)), strip = TRUE, main = " ", scales = list(x = list(at = c(seq(2.9, 
            52, by = 4.42)), labels = c("Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"), alternating = c(1, 
            rep(0, (nyr - 1))), tck = 0, cex = 0.7), y = list(at = c(0, 
            1, 2, 3, 4, 5, 6), labels = c("Domingo", "Segunda", 
